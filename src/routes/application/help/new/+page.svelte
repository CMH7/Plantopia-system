<script>
  ...
  $: crumbs = [
    ...
    {
      name: `Help #${$helpsList.length + 1}`,
      link: `/application/help/new`
    }
  ]
  ...
  const saveChanges = async () => {
    ...
    let form = document.getElementById('formAddHelp')
    // @ts-ignore
    ...
    if (result.type === 'success') {
      ...
      setTimeout(() => {
        $notif.message = 'Successfully add help!'
        $notif.show = true
...
<form id='formAddHelp' class="hidden" method="post" action="?/addHelp" use:enhance>
  <input name='id' type="text" bind:value={data.help.id}>
  ...
  <div class="w-full h-full flex justify-center items-center">
    <div class="card bg-white h-[20vh] relative w-96 shadow-xl overflow-hidden">
      <div class="poppins w-full px-5 pt-2 text-center">
        Are you sure you want to add this help?
      </div>
