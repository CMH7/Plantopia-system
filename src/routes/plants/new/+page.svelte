<script>
  ...
  let crumbs = [
    ...
    {
      name: `New Plant (${plant.common_name})`,
      link: `/plants/new`
    }
  ]
  ...
  const addNewSName = (e) => {
    ...
    if(plant.scientific_name.find(x => x.trim().toLowerCase() === newSname.trim().toLowerCase())) {
      notify(
        'warning',
    ...
    plant.scientific_name = [...plant.scientific_name, newSname]
    newSname = ''
  }
  ...
  const addNewOName = (e) => {
    ...
    if(plant.other_name.find(x => x.trim().toLowerCase() === newOname.trim().toLowerCase())) {
      notify(
        'warning',
    ...
    plant.other_name = [...plant.other_name, newOname]
    newOname = ''
  }
  
  const removeSName = (index) => {
    if(plant.scientific_name.length == 1) {
      notify(
        'warning',
    ...
    plant.scientific_name = plant.scientific_name.filter((x, i) => i != index)
  }
  
  const removeOName = (index) => {
    plant.other_name = plant.other_name.filter((x, i) => i != index)
  }
  ...
  const checkCName = () => {
    if(plant.common_name === '' || plant.common_name == null) {
      notify(
        'warning',
        'Invalid Name',
  ...
  const checkDesc = () => {
    if(plant.description === '' || plant.description == null) {
      notify(
        'warning',
        'Invalid Input',
  ...
  const saveChanges = async () => {
    ...
    if(hasImage) {
      ...
      const storageRef = ref(storage, `images/plants/${plant.docID}`);
    ...
    notify(
      'info',
      'Creating plant\'s fields',
      `Creating fields and data of plant (${plant.common_name})`,
      1,
      true,
      5000
    )
    await setDoc(
      doc(db, 'plants', plant.docID),
      {
      ...
      notify(
        'success',
        'Creating plant\'s fields',
        `Successfully created fields and data of plant (${plant.common_name})`,
        1,
        true,
        5000
      )
    ...
    adding = false
    closeModal()
    location.href = `/plants/plantinfo?docID=${plant.docID}`
  }

  onMount(() => {
    
    inputPrice = document.getElementById(`plant-price-${plant.docID}`);
    const maskOptions = {
    ...
    <div class="w-full flex justify-center gap-x-3">
      ...
      <div class="pl-28 w-1/3 flex flex-col gap-y-3">
        ...
        <div class="flex flex-col w-full">
          <label for="plant-cname-{plant.docID}" class="font-bold">Common name</label>
          <input id="plant-cname-{plant.docID}" bind:value={plant.common_name} on:change={() => checkCName()} type="text" class="input input-bordered w-full" />
        </div>
  
        <!-- Seasons -->
        <div class="flex flex-col w-full">
          <label for="plant-seasons-{plant.docID}" class="font-bold">Seasons</label>
          <select bind:value={plant.seasons} class="border-neutral-300 rounded-md w-full p-0 hide-scrollbar m-0" multiple>
        ...
        <!-- Image -->
        <div class="flex flex-col w-full">
          <label for="plant-image-{plant.docID}" class="font-bold">Image</label>
          <input bind:this={inputImage} on:change={inputImageOnChange} id="plant-image-{plant.docID}" type="file" class="file-input file-input-bordered w-full" />
        </div>
      ...
      <div class="pr-28 w-2/3 flex flex-col gap-y-3">
        <!-- Description -->
        <div class="flex flex-col w-full">
          <label for="plant-desc-{plant.docID}" class="font-bold">Description</label>
          <textarea id="plant-desc-{plant.docID}" bind:value={plant.description} on:change={() => checkDesc()} class="input input-bordered w-full h-[200px] resize-none hide-scrollbar" />
        </div>
        
        <!-- Care -->
        <div class="flex flex-col w-full">
          <label for="plant-care-{plant.docID}" class="font-bold">Care</label>
          <textarea id="plant-care-{plant.docID}" bind:value={plant.care} on:change={() => checkCare()} class="input input-bordered w-full h-[200px] resize-none hide-scrollbar" />
        </div>
        ...
        <div class="w-full flex gap-x-3">
          <!-- Scientific Names -->
          <div class="flex flex-col w-1/2">
            <label for="plant-sname-{plant.docID}" class="font-bold">Scientific names</label>
            <input id="plant-sname-{plant.docID}" bind:value={newSname} type="text" on:change|preventDefault={e => addNewSName()} class="input input-bordered w-full rounded-b-none" />
            ...
            <div on:click={() => showSNames = !showSNames} class="collapse w-full rounded-t-none bg-base-200 collapse-arrow">
              ...
              <div class="collapse-content">
                <div class="flex flex-col py-2">
                  {#each plant.scientific_name as sname, i}
          ...
          <!-- Other Names -->
          <div class="flex flex-col w-1/2">
            <label for="plant-oname-{plant.docID}" class="font-bold">Other names</label>
            <input id="plant-oname-{plant.docID}" bind:value={newOname} type="text" on:change|preventDefault={e => addNewOName()} class="input input-bordered w-full rounded-b-none" />
            <!-- svelte-ignore a11y-click-events-have-key-events -->
            <!-- svelte-ignore a11y-no-static-element-interactions -->
            <div on:click={() => showONames = !showONames} class="collapse w-full rounded-t-none bg-base-200 collapse-arrow">
              ...
              <div class="collapse-content">
                <div class="flex flex-col py-2">
                  {#each plant.other_name as oname, i}
                    <div class="{i % 2 == 0 ? '' : 'bg-neutral-300'} rounded-lg pl-3 flex items-center justify-between">
                      {oname}
